...
import { test } from "../animations/subNavInterface.animations.js";
import { Datas_Interface } from "../classes/Datas_Interface.js";
import { CardCategory } from "../components/Card.js";
import { HeaderInterface } from "../components/HeaderInterface.js";
import { interfaceDatasControllers } from "../controllers/categories.controllers.js";
...
export async function interfaceDataView() {
  ...
  main.appendChild(datas_interface.interface);
  let datas = new Datas("categories", result).display;
  ...
  // `;
  // document
  //   .querySelector("body > header > figure > svg")
  //   .addEventListener("click", () => datas_interface.reverseAnimation);
  // for (let c = 0; c < 4; c++)
  ...
  //~ ANIMATIONS
  let InterfaceAnims = datasInterfaceAnims();
  InterfaceAnims.forEach((anim) => anim.play());

  headerInterfaceAnims().forEach((anim) => anim.play());
  categoriesDataAnims().forEach((anim) => anim.play());

  interfaceDatasControllers(result);
}
