...
export class Datas {
  ...
  special() {
    ...
    if (this.datas.length < 4 || typeof this.datas != "object") return "";

    // //~ Create the next button
    let article = new HTMLElement("article", "next").element;
    let h1 = new HTMLElement("h1", null).text("See next");
    let svg = `
  ...
  async createSection() {
    ...
    for (const d of this.datas) {
      if (this.datas.indexOf(d) > 4 || typeof this.datas != "object") continue;
